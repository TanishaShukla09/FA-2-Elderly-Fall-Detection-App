...
See [RUBRIC_AUDIT.md](RUBRIC_AUDIT.md) for the FA-2 evidence checklist and final demonstration sequence.
...
- IMVIA/Le2i: https://www.kaggle.com/datasets/tuyenldvn/falldataset-imvia/data
...
| **Why Selected** | Real-time performance in-browser, 33 landmarks sufficient for fall-relevant joints, lightweight "lite" variant for fast inference |
...
| **Recall (weighted)** | **81.29%** |
| **F1-Score (weighted)** | **81.20%** |
...
The fall detection system uses a **3-tier fusion approach**:
...
**Live Application:** `[PASTE STREAMLIT CLOUD LINK HERE]` | **Repository:** `[PASTE GITHUB REPOSITORY LINK HERE]`
...
### WebRTC camera connection

The live camera uses a public STUN server by default. If a deployed app keeps
showing a WebRTC connection warning, the host network is likely blocking the
direct connection. Add TURN credentials to `.streamlit/secrets.toml` using
`.streamlit/secrets.toml.example` as the template; never commit real TURN
credentials.
...
| Accuracy | 81.29% | `screenshots/evaluation_results.json` |
| Precision (weighted) | 81.46% | `screenshots/evaluation_results.json` |
| Recall (weighted) | 81.29% | `screenshots/evaluation_results.json` |
| F1-Score (weighted) | 81.20% | `screenshots/evaluation_results.json` |
...
- [IMVIA/Le2i Fall Detection Dataset](https://www.kaggle.com/datasets/tuyenldvn/falldataset-imvia/data)
